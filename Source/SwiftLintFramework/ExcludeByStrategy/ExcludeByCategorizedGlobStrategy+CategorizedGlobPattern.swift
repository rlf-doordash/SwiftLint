...
extension ExcludeByCategorizedGlobStrategy {
    ...
    enum CategorizedGlobPattern {
        ...
        static func categorize(_ pattern: Configuration.ExcludePath) -> Self {
            ...
            // We use original because we want to avoid any prepended path for this configuration.
            // This will happen for prefix case as well
            if originalExcludePath.hasPrefix("**") {
                ...
                if suffix.rangeOfCharacter(from: globCharset) == nil {
                    ...
                    let data = CategorizedGlobPatternData(pattern: cleanSuffix,
                                                          configurationPath: pattern.originalConfigPath)
                    return .prefix(data)
                }
            }
            ...
            if originalExcludePath.hasSuffix("**") {
                ...
                if prefix.rangeOfCharacter(from: globCharset) == nil {
                    ...
                    let data = CategorizedGlobPatternData(pattern: cleanPrefix,
                                                          configurationPath: pattern.originalConfigPath)
                    return .suffix(data)
                }
            }
